...
First step will be to create a filter of all the first names from the names table:
...
```python
merged['RESIDENTIAL_SECONDARY_ADDR']=merged.RESIDENTIAL_SECONDARY_ADDR.fillna('NA') #filling in blank secondary addresses for grouping
merged.groupby(['RESIDENTIAL_ADDRESS1','RESIDENTIAL_SECONDARY_ADDR','RESIDENTIAL_ZIP']).ngroups
```
